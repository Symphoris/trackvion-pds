...
public class UseBulbsUpdate {
	...
	public int id;
		...
		public UseBulbsUpdate(int ourUpdate, int id) {
		...
		public boolean executeInscription() {
			...
			try {
				...
				while(result.next()) {
					...
					PreparedStatement stmt = connection.prepareStatement("UPDATE ConnectedBulbs_Objects SET TargetBulbBrightness = ? Where ID_ConnectedBulbs = ? ;");
