...
public class ConfHobsInsert {
	
	public int ourUpdate;
	public int id;
		
		public ConfHobsInsert(int ourUpdate, int id) {
		...
		public boolean executeInscription() {
			...
			try {
				...
				while(result.next()) {
					...
					PreparedStatement stmt = connection.prepareStatement("UPDATE ConnectedHobs_Objects SET MaxHobsTemperature = ? Where ID_ConnectedHobs = ? ;");
