...
public class UseBathtubUpdate {
	
	public int ourupdate;
	public int id;
		
		public UseBathtubUpdate(int ourupdate, int id) {
		...
		public boolean executeInscription() {
			...
			try {
				...
				while(result.next()) {
					...
					PreparedStatement stmt = connection.prepareStatement("UPDATE ConnectedBathtub_Objects SET TargetWaterTemperature = ? Where ID_ConnectedBathtub = ? ;");
