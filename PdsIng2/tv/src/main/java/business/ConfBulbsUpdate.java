...
public class ConfBulbsUpdate {
	
	public int ourUpdate;
	public int id;
		
		public ConfBulbsUpdate(int ourUpdate, int id) {
		...
		public boolean executeInscription() {
			...
			try {
				...
				while(result.next()) {
					...
					PreparedStatement stmt = connection.prepareStatement("UPDATE ConnectedBulbs_Objects SET MaxBulbBrightness = ? Where ID_ConnectedBulbs = ? ;");
