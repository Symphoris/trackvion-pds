...
public class UseBlindsUpdate {
	
	public int ourUpdate;
	public int id;
		
		public UseBlindsUpdate(int ourUpdate, int id) {
		...
		public boolean executeInscription() {
			...
			try {
				...
				while(result.next()) {
					...
					PreparedStatement stmt = connection.prepareStatement("UPDATE ConnectedBlinds_Objects SET TargetBlindPosition = ? Where ID_ConnectedBlinds = ? ;");
